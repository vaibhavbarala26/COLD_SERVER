...
const baseURL = process.env.Environment === "prod" ? "https://cold-server-bj3d.vercel.app/email/track" : "http://localhost:1042/email/track"
    ...
    try {
      ...
      if (email.links && email.links.length > 0) {
        email.links.forEach((link) => {
          emailLines.push(`<p><a href="${baseURL}=${email.recipientList}&campaignId=${id}&url=${encodeURIComponent(link.url)}">${link.label}</a></p>`);
